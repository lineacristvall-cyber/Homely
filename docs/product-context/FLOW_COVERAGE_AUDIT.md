...
**2026-09-13 · Proposed interaction design; no implemented application.** The current [editable v3 flowchart](flows/Homely-UI-Decision-Flows-v3.drawio) begins with a connected end-to-end decision flow and eight expanded multi-step journeys. Pages 00–08 use native boxes, conditional diamonds, labeled arrows and return loops. The 128 exact-state reference pages follow these journeys; they document each supported action, its effect and a linked precise next state. Read the integrated journeys first rather than navigating one state at a time.

[Overview PNG](flows/Homely-v3-journey-00.png) · [Seller/recheck PNG](flows/Homely-v3-journey-04.png) · [Dense research reference PNG](flows/Homely-v3-state-S08-RUNNING.png) · [Preview guide](flows/Homely-v3-README.md).

The finite contract contains 370 modeled actions across 24 screen templates and 128 reachable states. It is not a claim to enumerate every combination of retailer, device, location, simultaneous failure or future feature. Linked reference pages are supplemental detail, not a substitute for the multi-step journey flowcharts.

## Validation and scope

- Parsed final uncompressed draw.io XML. Every cell ID is unique within its page; all arrow endpoints and internal page links resolve.
- Checked each action ID against its actual source → action/effect → destination edges and exact linked state. All named states are reachable from S01.START and have next actions.
- Checked all page geometry for out-of-bounds shapes, overlapping shapes/labels and arrow segments passing through unrelated shapes. Text wrapping was estimated for overflow.
- Locally inspected full previews for overview, seller/recheck, privacy/offline and dense research reference. Full-resolution crops verified the required-quantity diamond and outlined decision/replacement boxes. Previews derive from final native XML geometry, not an unrelated illustration.
- No browser/computer automation or diagrams.net import was performed, at the user's request. Internal links use native draw.io object metadata; interactive import/render behavior remains for the user's import. Font rendering can vary by local installation.
- [Machine-readable validation](flows/Homely-v3-validation.json) and [action manifest](flows/Homely-v3-action-manifest.json) accompany the drawing. Structural checks support traceability; node counts do not prove usability.

## Corrections to the prior v3 draft

The earlier 129-page/335-action draft is preserved in [history](flows/history/Homely-UI-Decision-Flows-v3-draft.drawio) with its [audit](FLOW_COVERAGE_AUDIT-v3-draft.md). Rejected [v2](flows/Homely-UI-State-Atlas-v2.drawio), [v2 audit](FLOW_COVERAGE_AUDIT-v2.md), and [v1](flows/Homely-experience-v1.drawio) are preserved.

Original action IDs remain stable. The current ledger clarifies or splits branches with distinct effects: export retry versus cancel; comment post versus cancel; purge confirm versus abort; image removal versus undo; filter reset versus empty filter; desktop retry versus choosing another room. Added explicit brief-validation entry, cancellation and return routes, refresh-success/error outcomes, permission gating for desktop voice, and terminal-state exits. These are proposed UX corrections, not implemented behavior.

## Integrated journey guide

| Page | Multi-step journey | Main screens |
| --- | --- | --- |
| 00 | Room-to-decision overview with branch gates and returns | Cross-screen |
| 01 | Session, camera/manual capture, measurements, voice/typed input | S01–S07 |
| 02 | Brief validation, research control, partial/empty source recovery | S06–S10, S19 |
| 03 | Lead verification, exact SKU, render/geometry checks, comparison | S10–S14 |
| 04 | Client scope/approval, stock and cap recheck, seller return | S16–S18, S20–S21 |
| 05 | Hunt setup, pause/resume, meaningful changes, purchase history | S18–S21 |
| 06 | Conditional commission, reserve, release and reversal | S17, S21 |
| 07 | Offline reconciliation, caller return, privacy/export/deletion | S22–S23 |
| 08 | Laptop room/evidence/compare workspace | S15, S24 |

## Open assumptions

Canonical [CURRENT_CONTEXT.md](CURRENT_CONTEXT.md) takes precedence. Exact-variant availability and quantity evidence, source-specific freshness/expiry rules and source access rights still require validation. “Six” in example actions refers to the illustrative dining-chair brief; real requests use their own required count. Actual-product visualization fidelity and measured geometry must be tested; composites do not establish physical fit. Sharing roles, privacy/retention/purge semantics, monitoring cadence, source rights, commercial model and credit eligibility remain design decisions. Client approval never reserves stock. Monitoring requires permitted backend infrastructure; a closed PWA cannot reliably run the job itself. Ranking excludes commission.

## Screen/state coverage

| Screen | Template | Named states | Actions |
| --- | --- | ---: | ---: |
| S01 | Welcome & intent | 5 | 12 |
| S02 | Permissions at point of use | 3 | 13 |
| S03 | Project home & session | 4 | 13 |
| S04 | Room canvas & voice | 4 | 15 |
| S05 | Capture, edit & dimensions | 10 | 23 |
| S06 | Taste & inspiration | 5 | 14 |
| S07 | Structured brief & constraints | 5 | 15 |
| S08 | Research job & progress | 6 | 16 |
| S09 | No result & source gap | 3 | 9 |
| S10 | Findings & spoken briefing | 6 | 19 |
| S11 | Exact candidate evidence | 7 | 20 |
| S12 | Unconfirmed lead | 4 | 11 |
| S13 | Actual product in room | 6 | 17 |
| S14 | Mobile comparison | 4 | 14 |
| S15 | Desktop project canvas | 3 | 15 |
| S16 | Decision, budget & stock gate | 6 | 18 |
| S17 | External seller handoff & earned usage | 5 | 11 |
| S18 | Stock-change alert & replacement | 3 | 10 |
| S19 | Saved hunts & notification inbox | 7 | 19 |
| S20 | Client share, approval & revisions | 7 | 19 |
| S21 | Decision & purchase ledger | 6 | 13 |
| S22 | Account, privacy & data | 7 | 19 |
| S23 | Offline, reconnect & conflicts | 7 | 17 |
| S24 | Desktop research workspace | 5 | 18 |

## Complete action and destination ledger

Each ID is the `action-SNN-ANN` native box on its source-state page. Its outgoing arrow terminates at `destination-SNN-ANN`, whose native link points to the named next-state page.

### S01

| Action ID | From state | Control or event | Guard / effect | Destination |
| --- | --- | --- | --- | --- |
...
| S01-A12 | S01.LOADING | Cancel pending operation | Preserve prior version; discard late result for canceled request | S01.AUTH |
...
| Action ID | From state | Control or event | Guard / effect | Destination |
| --- | --- | --- | --- | --- |
| S04-A01 | S04.IDLE | Tap microphone without permission | Show point-of-use primer; do not record | S02.PRIMER |
| S04-A02 | S04.IDLE | Tap microphone with permission | Start visible recording with Stop control | S04.LISTENING |
...
| S04-A15 | S04.CORRECTING | Cancel correction | Keep provisional transcript for review | S04.TRANSCRIPT |
...
| S05-A17 | S05.DELETING | Undo image deletion | Restore removed original and selection | S05.PHOTO |
| S05-A18 | S05.PHOTO | Back / cancel capture | Keep saved image; discard unsaved edit | S04.IDLE |
| S05-A19 | S05.DELETING | Confirm removal | Remove selection recoverably; choose another image | S05.CAPTURE |
| S05-A20 | S05.CAPTURE | Take photo when camera allowed | Submit captured image to validation | S05.UPLOADING |
| S05-A21 | S05.UPLOAD_ERROR | Cancel upload | Keep original image and room draft | S04.IDLE |
| S05-A22 | S05.CAPTURE | Cancel capture | Return to room without adding an image | S04.IDLE |
| S05-A23 | S05.UPLOADING | Cancel pending operation | Preserve prior version; discard late result for canceled request | S04.IDLE |
...
| S06-A12 | S06.EMPTY | Retry inspiration upload | Preserve taste board | S06.UPLOADING |
| S06-A13 | S06.STYLE | Continue without inspiration | Keep aesthetic uncertainty explicit | S07.EDITING |
| S06-A14 | S06.UPLOADING | Cancel pending operation | Preserve prior version; discard late result for canceled request | S06.STYLE |
...
| S07-A13 | S07.EDITING | Review and validate brief | Validate all fields even if no hard cap was edited | S07.VALIDATING |
| S07-A14 | S07.REVIEW | Accept proposed voice changes | Commit explicit chips and validate before search | S07.VALIDATING |
| S07-A15 | S07.REVIEW | Cancel proposed changes | Preserve prior saved brief | S04.IDLE |
...
| S08-A16 | S08.PAUSED | Cancel paused job | Retain completed evidence in history | S08.CANCELLED |
...
| S10-A08 | S10.FILTERING | No matches after filtering | Show empty filtered set without discarding source results | S10.FILTER_EMPTY |
...
| S10-A17 | S10.FILTERING | Clear or cancel filters | Restore the prior ranked set | S10.RESULTS |
| S10-A18 | S10.REFRESHING | Fresh stock unchanged | Update verification timestamps | S10.RESULTS |
| S10-A19 | S10.REFRESHING | Refresh fails | Keep cached results visibly stale and show source gap | S10.PARTIAL |
...
| S11-A20 | S11.UNAVAILABLE | Back to findings | Keep failed verification visible in history | S10.RESULTS |
...
| S12-A10 | S12.UNKNOWN | Retry inquiry | Require permitted access; do not claim ready stock | S12.PENDING |
| S12-A11 | S12.UNKNOWN | Back to findings | Keep lead unresolved | S10.RESULTS |
...
| S13-A17 | S13.PROCESSING | Cancel pending operation | Preserve prior version; discard late result for canceled request | S11.DETAIL |
...
| S14-A09 | S14.COST_UNKNOWN | Ask for missing quote | Keep costs unknown and selection blocked | S12.LEAD |
...
| S14-A14 | S14.COST_UNKNOWN | Choose another option | Preserve cap; return to comparison | S14.COMPARE |
...
| S15-A04 | S15.ERROR | Retry room loading | Keep brief and original project version | S15.LOADING |
...
| S15-A10 | S15.CANVAS | Speak shortcut with permission | Use visible recording and transcript lifecycle | S04.LISTENING |
...
| S15-A14 | S15.ERROR | Choose another room | Return to projects with existing draft | S03.HOME |
| S15-A15 | S15.CANVAS | Speak shortcut without permission | Request microphone at point of use | S02.PRIMER |
...
| S16-A18 | S16.RECHECKING | Cancel pending operation | Preserve prior version; discard late result for canceled request | S16.BUDGET |
...
| S19-A18 | S19.SETTINGS | Cancel hunt setup or edits | Preserve previously saved hunt settings if any | S03.HOME |
| S19-A19 | S19.PAUSED | Back to project | Keep hunt paused | S03.HOME |
...
| S20-A09 | S20.COMMENTING | Post comment | Persist comment and notify owner | S20.REVIEW |
...
| S20-A18 | S20.COMMENTING | Cancel comment | Discard unsent comment; do not notify | S20.REVIEW |
| S20-A19 | S20.PREVIEW | Cancel sharing | No link created; retain saved decision | S16.BUDGET |
...
| S22-A10 | S22.EXPORT_ERROR | Retry export | Queue archive again without deleting data | S22.EXPORTING |
| S22-A11 | S22.SETTINGS | Delete photo/project recoverably | Show affected room, Undo/recovery period | S22.TRASH |
| S22-A12 | S22.TRASH | Restore deleted item | Recover item within retention period | S22.SETTINGS |
| S22-A13 | S22.SETTINGS | Request permanent purge | Action-time confirmation of irreversibility | S22.PURGE_CONFIRM |
| S22-A14 | S22.PURGE_CONFIRM | Confirm permanent purge | Purge selected data after explicit confirmation | S22.SETTINGS |
...
| S22-A17 | S22.EXPORT_ERROR | Cancel export retry | Keep account and existing data | S22.SETTINGS |
| S22-A18 | S22.TRASH | Keep item in trash | Keep recoverable until retention expires | S22.SETTINGS |
| S22-A19 | S22.PURGE_CONFIRM | Abort permanent purge | Delete nothing; preserve data | S22.SETTINGS |
...
| S24-A05 | S24.FILTERED | Inspect filtered candidate | Preserve filters and chosen exact variant | S11.DETAIL |
...
| S24-A16 | S24.FILTERED | Reset filters | Restore research workspace results | S24.WORKSPACE |
| S24-A17 | S24.REFRESHING | Stock and cost unchanged | Refresh evidence timestamps | S24.WORKSPACE |
| S24-A18 | S24.REFRESHING | Recheck failed | Keep stale evidence and disclose source gap | S24.EVIDENCE |
